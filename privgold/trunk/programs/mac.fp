...
//add in the dot of light and normal* diffuse mat*diffuse light
  ambient+=max(nDotL0,0.0)*gl_FrontLightProduct[0].diffuse;
  if (light_enabled[1]!=0)
    ambient+=gl_FrontLightProduct[1].ambient+max(nDotL1,0.0)*gl_FrontLightProduct[1].diffuse;
...
//add in the lDotR product
 specularity+=pow(max(lDotR0,0.0),gl_FrontMaterial.shininess)*gl_FrontLightProduct[0].specular;
  if (light_enabled[1]!=0)
    specularity+=pow(max(lDotR1,0.0),gl_FrontMaterial.shininess)*gl_FrontLightProduct[1].specular;
